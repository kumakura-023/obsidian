...
_sendGate.Release();
```
今度は「送信が終わるまで待つ(**Releaseされるまで）**」挙動になる。  
...
### 🧠 `await _sendGate.WaitAsync(token)` の意味

`SemaphoreSlim.WaitAsync()` は「部屋の定員に空きがあるなら入室してtrue、満室なら待つ（またはfalseを返す）」という信号機のような動作

#### 1. `await`：非同期的に待つ

スレッドをブロックせず、「他が終わるまで待つ」。  
（UIが固まらない）

#### 2. `_sendGate.WaitAsync()`：ロックを取得

他の処理が `_sendGate` を握っていたら、  
`Release()` されるまで待機する。

#### 3. `(token)`：キャンセルを監視

ここがポイント。  
引数の `token` には **`CancellationToken`** が入る。

---

### 🪫 token の中身（CancellationTokenの実体）

`token` は「外からキャンセルされるかもしれない」という  
**キャンセル要求を伝えるためのオブジェクト**。

たとえば、こうやって作られる：

`var cts = new CancellationTokenSource(); CancellationToken token = cts.Token;`

そして `.Cancel()` を呼ぶと、その `token` に「キャンセルされた」というフラグが立つ。

---

#### `_sendGate.WaitAsync(token)` の意味（直訳すると）

> 「他の送信が終わるのを待つが、  
> もしキャンセル要求が来たら途中で待機をやめて例外を投げる」

つまり、ユーザーが「停止」や「閉じる」ボタンを押して `cts.Cancel()` が呼ばれると、  
`WaitAsync(token)` は `OperationCanceledException` を投げて終了する。