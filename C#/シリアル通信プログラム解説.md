...
```C#
 frame.UpdateCrc();
 ushort[] wordsSnapshot = frame.Words.ToArray();
 byte[] bytesSnapshot = frame.ToByteArray();
 ushort crc = frame.Crc;
 SerialSettings? settings = _serialPort.CurrentSettings;

 DateTimeOffset startedAt = _clock.UtcNow;

 try
 {
     await _serialPort.WriteAsync(bytesSnapshot, cancellationToken).ConfigureAwait(false);
     await _serialPort.FlushAsync(cancellationToken).ConfigureAwait(false);
     DateTimeOffset completedAt = _clock.UtcNow;
     _lastSendCompletedAt = completedAt;
     return SendAttemptResult.CreateSuccess(startedAt, completedAt, delayApplied, settings, wordsSnapshot, bytesSnapshot, crc);
 }
```
...
await _serialPort.WriteAsync(payloadBytes, cancellationToken);
await _serialPort.FlushAsync(cancellationToken);

byte[] responseBytes = await ReadResponseAsync(request, cancellationToken);
string responseText = encoding.GetString(responseBytes);
```
