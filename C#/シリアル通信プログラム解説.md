...
else     _pollInterval = TimeSpan.FromSeconds(2); // 無効 or null → 2秒周期`
...
これは**タイマーを起動**して、「定期的にポート一覧を再取得する」仕組みを作っている。
...
`public Task RefreshAsync(...) {     latest = SerialPort.GetPortNames();     比較して変化があれば PortsChanged イベントを発火; }`
