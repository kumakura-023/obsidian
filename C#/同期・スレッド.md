...
private async void btnRun2_Click(object sender, EventArgs e)
{
    ...
    lblStatus.Text = "非UIで続き";
    await Task.Delay(500).ConfigureAwait(false); // ★UIコンテキストに戻さない

    // ここは非UIスレッドの可能性が高い。UIを直接触らない！
    var text = "続きは非UI";
...
public class DummyCatalog : IPortCatalog
{
    ...
    public Task RefreshAsync(CancellationToken ct)
    {
        ...
        {
            _ports = latest;
            PortsChanged?.Invoke(this, _ports); // ★プッシュ
        }
...
// ViewModel/フォーム側：プッシュ + プルのハイブリッド
private IPortCatalog _catalog = new DummyCatalog();

private void Form1_Load(object sender, EventArgs e)
{
    _catalog.PortsChanged += (s, ports) =>
        BeginInvoke((Action)(() => UpdatePorts(ports))); // ★プッシュで追従

    _ = RefreshPortsAsync(); // ★起動直後は“明示同期（プル）”で正を作る
}
